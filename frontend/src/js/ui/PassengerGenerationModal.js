export class PassengerGenerationModal {
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="modal-section chart-section">
            ...
            <div id="total-projected-demand" style="text-align: center; font-size: 1.1em; margin-top: 10px; font-weight: 500;">
                Total Projected Daily Passengers: <span id="total-pax-val">---</span>
            </div>
          </div>
    ...
    `;
        ...
        this.totalPaxVal = document.getElementById('total-pax-val');
    ...
    initChart() {
        ...
        for (let i = 0; i <= 1440; i += 30) {
            ...
            const timeStr = `${validHour.toString().padStart(2, '0')}:${(i % 60).toString().padStart(2, '0')}`;
            labels.push(timeStr);
                        ...
                        callbacks: {
                            label: function (context) {
                                ...
                                if (context.parsed.y !== null) {
                                    label += Math.round(context.parsed.y) + ' pa/min';
                                }
                                return label;
                            },
                            afterBody: function (context) {
                                if (context[0].parsed.y !== null) {
                                    const hourly = Math.round(context[0].parsed.y * 60);
                                    return `(~${hourly.toLocaleString()} pa/hour)`;
                                }
                                return '';
                            }
                        }
    ...
    updateTotalDisplay() {
        if (this.totalPaxVal) {
            const total = this.calculateTotalProjected();
            this.totalPaxVal.textContent = total.toLocaleString();
        }
    }
